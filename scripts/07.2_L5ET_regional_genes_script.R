...
write.csv(region_markers, here("markers", "L5ET_regional_markers.csv"))
...
#plot violin plot 
umap_genes <- genes_oi %>%
  filter(prop_diff > 0.4) %>% distinct(gene) %>% pull()
...
to_plot <- reductions$umap@cell.embeddings %>%
  ...
  left_join(
    seurat_obj@assays$RNA@data[umap_genes, ] %>%
      as_tibble(rownames = "gene") %>%
      gather(key = "sample_id", value = "expr", -gene),
    by = "sample_id"
  )

curated_genes <- c("KCNG2", "SHISA9", "RREB1", "EGFEM1P", "OR51E2",
                   "LAMA2", "LOC101929352", "EYA4", "HTR1F", "GRIK1", "FAM160A1", "HTR2C",
                   "KCTD8", "PPAPDC1A", "TRHDE", "ANK1", "DOK5", "PCDH11X")
...
p1 <- to_plot %>%
  ...
  mutate(region = region %>% as_factor() %>% fct_relevel(region_order),
         gene = gene %>% as_factor() %>% fct_relevel(curated_genes)) %>%
  ggplot() +
...
pdf(here("plots", "L5ET_region_markers_violin.pdf"),
    useDingbats = FALSE,
    height = 12,
    width = 6)
print(p1)
...
p1
  pdf(here("plots", "L5ET_GO_regional_genes.pdf"),
      useDingbats = FALSE,
      height = 10,
      width = 8)
print(p1)
